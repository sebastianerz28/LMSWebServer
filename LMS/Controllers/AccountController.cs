...
namespace LMS.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
            ...
            var studentUIDs = from s in db.Students
                              orderby s.UId descending
                              select new { uid = s.UId };
            var profUIDs = from p in db.Professors
                           orderby p.UId descending
                           select new { uid = p.UId };
            var adminUIDS = from a in db.Administrators
                            orderby a.UId descending
                            select new { uid = a.UId };

            string sID;
            if (studentUIDs.Count() > 0)
            {
                sID = studentUIDs.First().uid;
            }
            else
            {
                sID = "";
            ...
            string idOrder = IDS[2];
            string uID;
            if(idOrder != "")
            {
                if (int.TryParse(idOrder.Substring(1, idOrder.Length - 1), out int result))
                {
                    uID = "u" + (result+1).ToString("0000000");
                }
                else
                {
                    uID = "u0000000";
